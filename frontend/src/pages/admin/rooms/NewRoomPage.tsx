...
export function NewRoomPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setSubmitting(true)
    try {
      await createRoom({ name, memo: memo || null })
  ...
  return (
    <div className="mx-auto max-w-[600px] p-8">
      <h1 className="mb-6 text-[18px] font-bold">会議室 新規登録</h1>
      <form
      ...
      >
        ...
        <div>
          <label className="mb-1 block text-[11.5px] font-bold text-text-soft">メモ(任意)</label>
          <input
            className="w-full rounded-md border border-border px-3 py-2 text-sm"
            value={memo}
