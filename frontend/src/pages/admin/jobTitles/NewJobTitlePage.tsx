...
import { createJobTitle } from "../../../api/jobTitles"

export function NewJobTitlePage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "役職の登録に失敗しました")
    } finally {
      setSubmitting(false)
    }
  }
  ...
  return (
    <div className="mx-auto max-w-[600px] p-8">
      <h1 className="mb-6 text-[18px] font-bold">役職 新規登録</h1>
      <form
        onSubmit={handleSubmit}
        className="flex flex-col gap-4 rounded-[14px] border border-border bg-surface p-6"
      >
        <div>
          <label className="mb-1 block text-[11.5px] font-bold text-text-soft">役職名</label>
          <input
